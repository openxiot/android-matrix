...
import cc.openxiot.matrix.data.AppException
import cc.openxiot.matrix.data.api.UpdateApi
import cc.openxiot.matrix.util.Constants
import cc.openxiot.matrix.util.VersionCompare
...
class UpdateRepository(private val context: Context) {
    ...
    suspend fun check(): Result<CheckOutcome> = runCatching {
        ...
        // 清单必须指向**带版本号**的那个文件。指向 matrix-latest.apk 会让「清单说是哪一版」
    ...
    suspend fun sweepStale(): Unit = withContext(Dispatchers.IO) {
        updatesDir.listFiles()?.forEach { file ->
            ...
            val version = file.name.removePrefix("matrix-").removeSuffix(".apk")
            val diff = VersionCompare.compare(version, BuildConfig.VERSION_NAME)
            // 名字解析不出来（`matrix-latest.apk` 这种别名，或谁手工塞进来的东西）也删：
    ...
    private fun apkFileFor(version: String): File = File(updatesDir, "matrix-$version.apk")
    ...
    private companion object {
        ...
        const val LATEST_ALIAS = "matrix-latest.apk"
