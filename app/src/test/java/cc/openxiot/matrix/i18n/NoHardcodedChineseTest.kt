...
class NoHardcodedChineseTest {
     ...
     * `cc/openxiot/matrix/ui/modbus`。不剥前缀，白名单就一条都匹配不上 —— 表现是门禁
     * 从头红到尾、把 400 多条全倒出来（`pending 的每一条都真的挂在文件上` 那条测试专门
     * 防它复活）。
     */
    private val PKG_PREFIX = "cc/openxiot/matrix/"
     ...
     * [cc.openxiot.matrix.ui.modbus.fieldBaseName] 匹配的「读 / 写」是**用户敲进点表的
